...
Cleans raw data and engineers features for the pricing engine.
"""
import numpy as np
import pandas as pd


def process_data(sales_df: pd.DataFrame,
                 competitor_df: pd.DataFrame) -> pd.DataFrame:
    """
    Merge sales and competitor data, clean, and add engineered features.

    Returns a single DataFrame indexed by date with all features.
    """
    sales = sales_df.copy()
    comp = competitor_df.copy()

    # Drop DB auto-increment id columns if present
    for col in ["id"]:
    ...
    # Drop pre-existing competitor_avg_price (NULL from DB schema) to avoid merge conflict
    if "competitor_avg_price" in sales.columns:
        sales = sales.drop(columns=["competitor_avg_price"])

    # ── 1. Compute daily average competitor price ────────────────────────
    comp_avg = (
        comp.groupby("date")["price"]
        .mean()
        .reset_index()
        .rename(columns={"price": "competitor_avg_price"})
    )
    ...
    # Forward-fill any nulls from merge gaps
    df["competitor_avg_price"] = df["competitor_avg_price"].ffill().bfill()

    # ── 4. Feature engineering ───────────────────────────────────────────
    # Price difference: positive → we are cheaper
    ...
    # Demand trend: 7-day percentage change of the 7-day MA
    ...
    df["stock_category"] = pd.cut(
        df["stock"],
        bins=[-1, 100, 350, float("inf")],
        labels=["Low", "Medium", "High"],
    )

    # Price index: our price / competitor avg
...
def get_latest_features(processed_df: pd.DataFrame) -> dict:
    """Extract the most recent row's features as a dict for the pricing engine."""
